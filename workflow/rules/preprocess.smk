...
validate(
    config,
    os.path.join(workflow.current_basedir, "../../config/preprocess.schema.yaml"),
)
...
rule remove_primers:
    ...
    params:
        outstring=lambda wc, input, output: " -o {} -p {} ".format(
            output.R1, output.R2
        )
        if len(input.reads) > 1
        else " -o {}".format(output.R1),
        primerstring=lambda wc, input, output: " -g correct={F} -g flipped={R} -G flipped={F} -G correct={R}".format(
